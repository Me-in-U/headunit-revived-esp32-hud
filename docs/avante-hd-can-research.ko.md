...
1. **Windows OBD/CAN live analysis**: 레이아웃 에디터의 `Connection`, `OBD Analysis`, `CAN Analysis` 탭에서 OBD BLE와 CANable USB/SLCAN을 연결하고, 표준 PID와 raw CAN 후보를 같은 preview simulation에서 확인한다.
2. **확장 PID probe 작성**: `OBD Analysis` 탭에서 OBDb 후보를 `obd_probe_commands`에 추가하고 raw response, decoded value, timeout을 확인한다.
3. **HUD state 확장**: 검증된 값만 `vehicle.*`, `warnings.*`, `dtc.*` 같은 Pi 로컬 상태로 추가한다.
4. **확정 CAN signal 작성**: 기어 위치, 방향지시등, cluster 상태처럼 OBD로 안 잡히는 항목은 `CAN Analysis` 탭에서 frame/byte 후보를 좁히고 차량 profile `can_signals`에 `confirmed:true`로 저장한다.
...
| 목적 | 장비 | 비고 |
| --- | --- | --- |
| 표준/확장 OBD | Vgate iCar Pro 2S, BLE/BT ELM327 | Windows 에디터에서 BLE 연결/분석 후 Pi runtime mapping으로 배포 |
| passive CAN sniff | CANable USB/SLCAN, CANtact, USBtin, CANedge, comma panda | Windows 에디터는 CANable SLCAN을 우선 지원, listen-only 중요 |
| Pi 기반 runtime 확인 | Raspberry Pi + CANable/SocketCAN | 확정된 `can_signals`와 runtime stale/error 동작 검증 |
